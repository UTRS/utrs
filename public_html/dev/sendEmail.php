	...
	if(isset($_POST['submit'])){
		try{
			...
			if ($_POST['template'] == "") {
				...
				Log::ircNotification("\x033,0Email said to user \x032,0" . $appeal->getCommonName() . "\x033,0 by \x032,0" . $admin->getUsername());
			} else {
				$log->addNewItem("Sent email to user using " . $_POST['template'] . " template", 1);
				Log::ircNotification("\x033,0Email said to user \x032,0" . $appeal->getCommonName() . "\x033,0 using template \x032,0" . $_POST['template'] . "\x033,0 by \x032,0" . $admin->getUsername());
			}
			...
			if (isset($_POST['statusUser']) || isset($_POST['statusClose'])) {
				...
				$appeal->update();
				Log::ircNotification("\x033,0Status changed for \x032,0" . $appeal->getCommonName() . "\x033,0 (\x032,0 " . $appeal->getID() . "\x033,0 ) to \x032,0 " . $appeal->getStatus() . " \x033,0by \x032,0" . $appeal->getHandlingAdmin()->getUsername() . "\x033,0 URL: http://toolserver.org/~unblock/dev/appeal.php?id=" . $appeal->getID());
			}
			$success = true;
		}
		catch(Exception $e){
