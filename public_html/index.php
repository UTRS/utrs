...
//Account name
echo '<label id="accountNameLabel" for="accountName" class="required" hidden>What is the name of your account?</label> <input id="accountName" type="text" name="appeal_wikiAccountName" value="' . posted('appeal_wikiAccountName') . '" hidden><br /><br />';//Hidden text field, to be flipped by js
//What has been blocked
echo '<label id="autoBlockLabel" for="autoBlock" class="required" hidden>What has been blocked?</label> &#09; <input id="autoBlockN" type="radio" name="appeal_autoblock" value="0" onClick="accountBlock()" ' . (isset($_POST['appeal_autoblock']) ? ($autoBlock  ? 'checked="checked"' : '') : "") . ' hidden> My Account &#09; <input id="registeredN" type="radio" name="appeal_hasAccount" value="0" onClick="noAccountBlock()" ' . (isset($_POST['appeal_autoblock']) ? (!$appeal_autoblock ? 'checked="checked"' : '') : '') . ' hidden> Something else (my account is <b>NOT</b> directly blocked<br /><br />'; //Hidden text field, to be flipped by js
//New autoblock field
echo '<label id="autoblockIDLabel" for="autoblockID" class="required" hidden>What is the reference number associated with your block?</label> <input id="autoblockID" type="text" name="appeal_autoblockID" value="' . posted('appeal_autoblockID') . '" hidden><br /><br />';//Hidden text field, to be flipped by js
echo '<label id="accountNameLabel" for="accountName" hidden>We may be able to create an account for you which you can use to avoid problems like this in the future. If you would like for us to make an account for you, please enter the username you would like to use here.</label> <input id="accountName" type="text" name="appeal_wikiAccountName" value="' . posted('appeal_wikiAccountName') . '" hidden><br /><br />';//Hidden text field, to be flipped by js
