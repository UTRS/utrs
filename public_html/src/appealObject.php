...
class Appeal extends Model {
   ...
   */
   ...
   public function verifyNoPublicAppeal($username) {
      ...
      $checkFound = False;
      $reviewSearch = preg_match("^.*\{\{unblock.*reviewed^",strtolower($data["query"]["pages"]["{$pageid}"]["revisions"]["*"]));
      if ($reviewSearch !== 0) {
        if (count(preg_match("^.*\{\{unblock.*reviewed^",strtolower($data["query"]["pages"][$pageid]["revisions"]["*"]))<count(preg_match("^.*\{\{(U|u)nblock^",$data["query"]["pages"][$pageid]["revisions"]["*"])))) {
          return False;
        }
      }
      else { 
