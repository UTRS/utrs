...
class Log {
	...
	public function getSmallHTML($higherPerms) {
		...
		for ($i = 0; $i < count($this->log); $i++) {
			...
			$username = ($data['commentUser']) ? "<a href=\"userMgmt.php?userId=" . $data['commentUser'] . "\">" . WikiUser::getUserById($data['commentUser'])->getUserName() . "</a>" : Appeal::getAppealByID($data['appealID'])->getCommonName();
